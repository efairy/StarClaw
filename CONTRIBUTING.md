...
- **Check [Open Issues](https://github.com/efairy/StarClaw/issues)** and any [Projects](https://github.com/efairy/StarClaw/projects) or roadmap labels.
...
StarClaw is designed to be **extensible**: you can add models, channels, Skills, and more. Below are the main contribution areas we care about.

---

### Adding New Models / Model Providers

StarClaw supports **multiple model backends**: cloud APIs (e.g. DashScope, ModelScope), **Ollama**, and local backends (**llama.cpp**, **MLX**). You can contribute in two ways:
...
1. **Provider definition** (in `src/starclaw/providers/registry.py` or equivalent):
...
2. **Chat model class** (if the API is not OpenAI-compatible):
   - Implement a class inheriting from `agentscope.model.ChatModelBase` (or StarClaw’s local/remote wrappers where applicable).
   - Support streaming and non-streaming if the agent uses both; respect `tool_choice` and tools API if used.
   - Register the class in the registry’s chat model map so the runtime can resolve it by name (see `_CHAT_MODEL_MAP` in `src/starclaw/providers/registry.py`).
...
Channels are how StarClaw talks to **DingTalk, Feishu, QQ, Discord, iMessage**, etc. You can add a new channel so StarClaw can work with your favorite IM or bot platform.
...
- **Implementation:** Implement a **subclass of `BaseChannel`** (in `src/starclaw/app/channels/base.py`):
...
- **Discovery:** Built-in channels are registered in `src/starclaw/app/channels/registry.py`. **Custom channels** are loaded from the working directory: place a module (e.g. `custom_channels/telegram.py` or a package `custom_channels/telegram/`) that defines a `BaseChannel` subclass with a `channel` attribute.
- **CLI:** Users install/add channels with:
  - `starclaw channels install <key>` — create a template or copy from `--path` / `--url`
  - `starclaw channels add <key>` — install and add to config
  - `starclaw channels remove <key>` — remove custom channel from `custom_channels/`
  - `starclaw channels config` — interactive config
...
**Skills** define what StarClaw can do: cron, file reading, PDF/Office, news, browser, etc. We welcome **broadly useful** base skills (productivity, documents, communication, automation) that fit the majority of users.
...
- **Location:** Built-in skills live under `src/starclaw/agents/skills/<skill_name>/`. The app merges built-in and user **customized_skills** from the working dir into **active_skills**; no extra registration is needed beyond placing a valid `SKILL.md` in a directory.
...
- **Skills Hub:** StarClaw supports importing skills from a community hub (e.g. ClawHub). If you want your skill to be installable via hub, follow the same `SKILL.md` + `references/`/`scripts/` layout and the hub’s packaging format.
...
StarClaw aims to run on **Windows**, **Linux**, and **macOS**. Contributions that improve support on a specific platform are welcome.
...
- **Install and run:** One-line install (`install.sh`), `pip` install, and `starclaw init` / `starclaw app` should work (or be clearly documented) on each supported platform. Fixes to install or startup on a given OS are valuable.
...
- **MCP (Model Context Protocol):** StarClaw supports runtime **MCP tool** discovery and hot-plug. Contributing new MCP servers or tools (or docs on how to attach them) helps users extend the agent without changing core code.
...
- **Discussions:** [GitHub Discussions](https://github.com/efairy/StarClaw/discussions)
- **Bugs and features:** [GitHub Issues](https://github.com/efairy/StarClaw/issues)
- **Community:** DingTalk group (see [README](README.md)) and [Discord](https://discord.gg/eYMpfnkG8h)

Thank you for contributing to StarClaw. Your work helps make it a better assistant for everyone. 🐾
