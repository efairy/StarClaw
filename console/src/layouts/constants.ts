...
export const PYPI_URL = "https://pypi.org/pypi/starclaw/json";

export const GITHUB_URL = "https://github.com/efairy/StarClaw" as const;
...
export const getDocsUrl = (lang: string): string =>
  `https://efairy.github.io/StarClaw/docs/intro?lang=${getWebsiteLang(lang)}`;

export const getFaqUrl = (lang: string): string =>
  `https://efairy.github.io/StarClaw/docs/faq?lang=${getWebsiteLang(lang)}`;

export const getReleaseNotesUrl = (lang: string): string =>
  `https://efairy.github.io/StarClaw/release-notes?lang=${getWebsiteLang(lang)}`;
...
docker run -p 127.0.0.1:8088:8088 -v starclaw-data:/app/working agentscope/starclaw:latest
\`\`\`

升级后重启服务 starclaw app。`,

  ru: `### Как обновить StarClaw

Чтобы обновить StarClaw, выберите способ в зависимости от типа установки:
...
После обновления перезапустите сервис с помощью \`starclaw app\`.`,

  en: `### How to update StarClaw
