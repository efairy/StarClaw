...
- Builds the console frontend (`console/`), copies `console/dist` to `src/starclaw/console/dist`, then builds the wheel. Output: `dist/*.whl`.
...
- Default tag: `starclaw:latest`. Uses `deploy/Dockerfile` (multi-stage: builds console then Python app).
- Example: `bash scripts/docker_build.sh myreg/starclaw:v1 --no-cache`.
