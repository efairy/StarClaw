# StarClaw Installer for Windows (self-contained: includes uv download via GitHub)
# Usage: irm <url>/install.ps1 | iex
#    or: .\install.ps1 [-Version X.Y.Z] [-FromSource] [-SourceDir DIR]
#                            [-Extras "llamacpp,mlx"] [-UvPath PATH]
#
# Installs StarClaw into ~/.starclaw with a uv-managed Python environment.
...
# ── Defaults ──────────────────────────────────────────────────────────────────
$StarClawHome     = if ($env:STARCLAW_HOME) { $env:STARCLAW_HOME } else { Join-Path $HOME ".starclaw" }
$StarClawVenv     = Join-Path $StarClawHome "venv"
$StarClawBin      = Join-Path $StarClawHome "bin"
$PythonVersion = "3.12"
$StarClawRepo     = "https://github.com/efairy/StarClaw.git"

# ── Colors ────────────────────────────────────────────────────────────────────
function Write-Info { param([string]$Message) Write-Host "[starclaw] " -ForegroundColor Green  -NoNewline; Write-Host $Message }
function Write-Warn { param([string]$Message) Write-Host "[starclaw] " -ForegroundColor Yellow -NoNewline; Write-Host $Message }
function Write-Err  { param([string]$Message) Write-Host "[starclaw] " -ForegroundColor Red    -NoNewline; Write-Host $Message }
function Stop-WithError { param([string]$Message) Write-Err $Message; exit 1 }

# ── Help ──────────────────────────────────────────────────────────────────────
if ($Help) {
    @"
StarClaw Installer for Windows
...
Environment:
  STARCLAW_HOME            Installation directory (default: ~/.starclaw)
"@
    exit 0
}

Write-Host "[starclaw] " -ForegroundColor Green -NoNewline
Write-Host "Installing StarClaw into " -NoNewline
Write-Host "$StarClawHome" -ForegroundColor White
...
function Prepare-Console {
    ...
    $consoleDest = Join-Path $RepoDir "src\starclaw\console"
...
$VenvStarClaw = Join-Path $StarClawVenv "Scripts\starclaw.exe"

if ($FromSource) {
    if ($SourceDir) {
        $SourceDir = (Resolve-Path $SourceDir).Path
        Write-Info "Installing StarClaw from local source: $SourceDir"
        ...
        if (-not (Get-Command git -ErrorAction SilentlyContinue)) {
            Stop-WithError "git is required for -FromSource without a local directory. Please install Git from https://git-scm.com/ or pass a local path: .\install.ps1 -FromSource -SourceDir C:\path\to\StarClaw"
        }
        Write-Info "Installing StarClaw from source (GitHub)..."
        $cloneDir = Join-Path $env:TEMP "starclaw-install-$(Get-Random)"
        try {
            git clone --depth 1 $StarClawRepo $cloneDir
    ...
    $package = "starclaw"
    if ($Version) { $package = "starclaw==$Version" }

    Write-Info "Installing ${package}${ExtrasSuffix} from PyPI..."
    uv pip install "${package}${ExtrasSuffix}" --python $VenvPython --prerelease=allow --quiet --refresh-package starclaw
    if ($LASTEXITCODE -ne 0) { Stop-WithError "Installation failed" }
}

# Verify the CLI entry point exists
if (-not (Test-Path $VenvStarClaw)) { Stop-WithError "Installation failed: starclaw CLI not found in venv" }

Write-Info "StarClaw installed successfully"

# Check console availability (for PyPI installs, check the installed package)
if (-not $script:ConsoleAvailable) {
    $consoleCheck = & $VenvPython -c "import importlib.resources, starclaw; p=importlib.resources.files('starclaw')/'console'/'index.html'; print('yes' if p.is_file() else 'no')" 2>&1
    if ($consoleCheck -eq "yes") { $script:ConsoleAvailable = $true }
}

# ── Step 4: Create wrapper scripts ───────────────────────────────────────────
New-Item -ItemType Directory -Path $StarClawBin -Force | Out-Null

$wrapperPath = Join-Path $StarClawBin "starclaw.ps1"
$wrapperContent = @'
# StarClaw CLI wrapper — delegates to the uv-managed environment.
$ErrorActionPreference = "Stop"

$StarClawHome = if ($env:STARCLAW_HOME) { $env:STARCLAW_HOME } else { Join-Path $HOME ".starclaw" }
$RealBin   = Join-Path $StarClawHome "venv\Scripts\starclaw.exe"

if (-not (Test-Path $RealBin)) {
    Write-Error "StarClaw environment not found at $StarClawHome\venv"
...
$cmdWrapperPath = Join-Path $StarClawBin "starclaw.cmd"
$cmdWrapperContent = @"
@echo off
REM StarClaw CLI wrapper — delegates to the uv-managed environment.
set "STARCLAW_HOME=%STARCLAW_HOME%"
if "%STARCLAW_HOME%"=="" set "STARCLAW_HOME=%USERPROFILE%\.starclaw"
set "REAL_BIN=%STARCLAW_HOME%\venv\Scripts\starclaw.exe"
if not exist "%REAL_BIN%" (
    echo Error: StarClaw environment not found at %STARCLAW_HOME%\venv >&2
...
if (-not $isAlreadyAdded) {
    ...
    try {
        ...
        Write-Host ""
        Write-Host "ACTION REQUIRED: You must manually add the path to use StarClaw."
...
# ── Done ──────────────────────────────────────────────────────────────────────
Write-Host ""
Write-Host "StarClaw installed successfully!" -ForegroundColor Green
Write-Host ""

Write-Host "  Install location:  " -NoNewline; Write-Host "$StarClawHome" -ForegroundColor White
...
Write-Host ""
Write-Host "  starclaw init" -ForegroundColor White -NoNewline; Write-Host "       # first-time setup"
Write-Host "  starclaw app"  -ForegroundColor White -NoNewline; Write-Host "        # start StarClaw"
Write-Host ""
Write-Host "To upgrade later, re-run this installer."
Write-Host "To uninstall, run: " -NoNewline
Write-Host "starclaw uninstall" -ForegroundColor White
