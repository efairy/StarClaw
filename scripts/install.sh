...
info()  { printf "${GREEN}[starclaw]${RESET} %s\n" "$*"; }
warn()  { printf "${YELLOW}[starclaw]${RESET} %s\n" "$*"; }
error() { printf "${RED}[starclaw]${RESET} %s\n" "$*" >&2; }
die()   { error "$@"; exit 1; }

# ── Defaults ──────────────────────────────────────────────────────────────────
STARCLAW_HOME="${STARCLAW_HOME:-$HOME/.starclaw}"
STARCLAW_VENV="$STARCLAW_HOME/venv"
STARCLAW_BIN="$STARCLAW_HOME/bin"
PYTHON_VERSION="3.12"
STARCLAW_REPO="https://github.com/efairy/StarClaw.git"
...
printf "${GREEN}[starclaw]${RESET} Installing StarClaw into ${BOLD}%s${RESET}\n" "$STARCLAW_HOME"
...
if [ -d "$STARCLAW_VENV" ]; then
    info "Existing environment found, upgrading..."
else
    info "Creating Python $PYTHON_VERSION environment..."
fi

uv venv "$STARCLAW_VENV" --python "$PYTHON_VERSION" --quiet

# Verify the venv was created
[ -x "$STARCLAW_VENV/bin/python" ] || die "Failed to create virtual environment"
info "Python environment ready ($("$STARCLAW_VENV/bin/python" --version))"
...
## Ensure console frontend assets are in src/starclaw/console/ for source installs.
...
prepare_console() {
    ...
    local console_dest="$repo_dir/src/starclaw/console"
...
cleanup_console() {
    ...
    if [ "$_CONSOLE_COPIED" = 1 ]; then
        rm -rf "$repo_dir/src/starclaw/console/"*
    fi
}

if [ "$FROM_SOURCE" = true ]; then
    if [ -n "$SOURCE_DIR" ]; then
        info "Installing StarClaw from local source: $SOURCE_DIR"
        prepare_console "$SOURCE_DIR"
        info "Installing package from source..."
        uv pip install "${SOURCE_DIR}${EXTRAS_SUFFIX}" --python "$STARCLAW_VENV/bin/python" --prerelease=allow --index-url "$PYPI_MIRROR"
        cleanup_console "$SOURCE_DIR"
    else
        info "Installing StarClaw from source (GitHub)..."
        CLONE_DIR="$(mktemp -d)"
        trap 'rm -rf "$CLONE_DIR"' EXIT
        git clone --depth 1 "$STARCLAW_REPO" "$CLONE_DIR"
        prepare_console "$CLONE_DIR"
        info "Installing package from source..."
        uv pip install "${CLONE_DIR}${EXTRAS_SUFFIX}" --python "$STARCLAW_VENV/bin/python" --prerelease=allow --index-url "$PYPI_MIRROR"
        # CLONE_DIR is cleaned up by trap; no need for cleanup_console
    fi
else
    PACKAGE="starclaw"
    if [ -n "$VERSION" ]; then
        PACKAGE="starclaw==$VERSION"
    fi

    info "Installing ${PACKAGE}${EXTRAS_SUFFIX} from PyPI..."
    uv pip install "${PACKAGE}${EXTRAS_SUFFIX}" --python "$STARCLAW_VENV/bin/python" --prerelease=allow --quiet --index-url "$PYPI_MIRROR" --refresh-package starclaw
fi

# Verify the CLI entry point exists
[ -x "$STARCLAW_VENV/bin/starclaw" ] || die "Installation failed: starclaw CLI not found in venv"
info "StarClaw installed successfully"
...
if [ "$_CONSOLE_AVAILABLE" = 0 ]; then
    ...
    CONSOLE_CHECK="$("$STARCLAW_VENV/bin/python" -c "import importlib.resources, starclaw; p=importlib.resources.files('starclaw')/'console'/'index.html'; print('yes' if p.is_file() else 'no')" 2>/dev/null || echo 'no')"
...
mkdir -p "$STARCLAW_BIN"

cat > "$STARCLAW_BIN/starclaw" << 'WRAPPER'
#!/usr/bin/env bash
# StarClaw CLI wrapper — delegates to the uv-managed environment.
set -euo pipefail

STARCLAW_HOME="${STARCLAW_HOME:-$HOME/.starclaw}"
REAL_BIN="$STARCLAW_HOME/venv/bin/starclaw"
...
chmod +x "$STARCLAW_BIN/starclaw"
info "Wrapper created at $STARCLAW_BIN/starclaw"

# ── Step 5: Update PATH in shell profile ─────────────────────────────────────
PATH_ENTRY="export PATH=\"\$HOME/.starclaw/bin:\$PATH\""

add_to_profile() {
    local profile="$1"
    if [ -f "$profile" ] && grep -qF '.starclaw/bin' "$profile"; then
...
# ── Done ──────────────────────────────────────────────────────────────────────
echo ""
printf "${GREEN}${BOLD}StarClaw installed successfully!${RESET}\n"
echo ""

# Install summary
printf "  Install location:  ${BOLD}%s${RESET}\n" "$STARCLAW_HOME"
printf "  Python:            ${BOLD}%s${RESET}\n" "$("$STARCLAW_VENV/bin/python" --version 2>&1)"
...
echo "Then run:"
echo ""
printf "  ${BOLD}starclaw init${RESET}       # first-time setup\n"
printf "  ${BOLD}starclaw app${RESET}        # start StarClaw\n"
echo ""
printf "To upgrade later, re-run this installer.\n"
printf "To uninstall, run: ${BOLD}starclaw uninstall${RESET}\n"
