# StarClaw Desktop packaging scripts
...
# Output: dist/StarClaw-Setup-<version>.exe
# Creates two launchers:
#   - StarClaw Desktop.vbs (silent, no console window)
#   - StarClaw Desktop (Debug).bat (shows console for troubleshooting)
...
APP_ENV="$(pwd)/dist/StarClaw.app/Contents/Resources/env"
...
When you **double-click** the .app and nothing appears, the launcher writes stderr/stdout to `~/.starclaw/desktop.log`. Inspect that file for errors.
...
When users download the StarClaw macOS app (e.g. from Releases) as a `.app` (in a zip), macOS may show: *"Apple cannot verify that 'StarClaw' contains no malicious software"*. The app is not notarized. They can still open it as follows:

- **Right-click to open (recommended)**
  Right-click (or Control+click) the StarClaw app → **Open** → in the dialog click **Open** again. Gatekeeper will allow it; after that double-click works as usual.

- **Allow in System Settings**
  If still blocked, go to **System Settings → Privacy & Security**, find the message like *"StarClaw was blocked because it is from an unidentified developer"*, and click **Open Anyway** or **Allow**.

- **Remove quarantine attribute (not recommended for most users)**
  In Terminal: `xattr -cr /Applications/StarClaw.app` (or the path to the `.app` after unzipping). This clears the download quarantine flag; less safe than right-click → Open.
...
| File | Description |
|------|-------------|
| `build_common.py` | Create temporary conda env, install `starclaw[full]` from a wheel, conda-pack; produces archive. |
| `build_macos.sh` | One-click: build wheel → build_common → unpack into StarClaw.app; optional zip. |
| `build_win.ps1` | One-click: build wheel → build_common → unpack → create VBS/BAT launchers → makensis installer. |
| `starclaw_desktop.nsi` | NSIS script: pack `dist/win-unpacked`, add icons, and create shortcuts. |
