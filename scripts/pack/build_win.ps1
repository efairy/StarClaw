...
$Archive = Join-Path $Dist "starclaw-env.zip"
$Unpacked = Join-Path $Dist "win-unpacked"
$NsiPath = Join-Path $PackDir "starclaw_desktop.nsi"
...
$VersionFile = Join-Path $RepoRoot "src\starclaw\__version__.py"
$CurrentVersion = ""
...
$RunWheelBuild = $true
if ($CurrentVersion) {
  $wheelGlob = Join-Path $Dist "starclaw-$CurrentVersion-*.whl"
  ...
  if ($existingWheels.Count -gt 0) {
    ...
    $oldWheels = Get-ChildItem -Path (Join-Path $Dist "starclaw-*.whl") -ErrorAction SilentlyContinue
...
$LauncherBat = Join-Path $EnvRoot "StarClaw Desktop.bat"
@"
@echo off
cd /d "%~dp0"
...
REM Log level: env var STARCLAW_LOG_LEVEL or default to "info"
if not defined STARCLAW_LOG_LEVEL set "STARCLAW_LOG_LEVEL=info"

REM Set SSL certificate paths for packaged environment
REM Use temp file to avoid for /f blocking issue in bat scripts
set "CERT_TMP=%TEMP%\starclaw_cert_%RANDOM%.txt"
...
if not exist "%USERPROFILE%\.starclaw\config.json" (
  "%~dp0python.exe" -u -m starclaw init --defaults --accept-security
)
"%~dp0python.exe" -u -m starclaw desktop --log-level %STARCLAW_LOG_LEVEL%
"@ | Set-Content -Path $LauncherBat -Encoding ASCII

# Debug launcher .bat (shows console)
$DebugBat = Join-Path $EnvRoot "StarClaw Desktop (Debug).bat"
@"
@echo off
cd /d "%~dp0"
...
REM Debug mode: use debug log level by default (can override with STARCLAW_LOG_LEVEL)
if not defined STARCLAW_LOG_LEVEL set "STARCLAW_LOG_LEVEL=debug"
...
echo ====================================
echo StarClaw Desktop - Debug Mode
echo ====================================
echo Working Directory: %cd%
echo Python: "%~dp0python.exe"
echo PATH: %PATH%
echo Log Level: %STARCLAW_LOG_LEVEL%
...
echo [Launch] Starting StarClaw Desktop with log-level=%STARCLAW_LOG_LEVEL%...
echo Press Ctrl+C to stop
echo.
"%~dp0python.exe" -u -m starclaw desktop --log-level %STARCLAW_LOG_LEVEL%
echo.
echo [Exit] StarClaw Desktop closed
pause
"@ | Set-Content -Path $DebugBat -Encoding ASCII

# VBScript launcher (no console window)
$LauncherVbs = Join-Path $EnvRoot "StarClaw Desktop.vbs"
@"
Set WshShell = CreateObject("WScript.Shell")
batPath = CreateObject("Scripting.FileSystemObject").GetParentFolderName(WScript.ScriptFullName) & "\StarClaw Desktop.bat"
...
if (-not $Version) {
  ...
  try {
    $Version = (& (Join-Path $EnvRoot "python.exe") -c "from importlib.metadata import version; print(version('starclaw'))" 2>&1) -replace '\s+$', ''
...
Write-Host "[build_win] STARCLAW_VERSION=$Version OUTPUT_EXE will be under $Dist"
$OutInstaller = Join-Path (Join-Path $RepoRoot $Dist) "StarClaw-Setup-$Version.exe"
