# -*- coding: utf-8 -*-
"""StarClaw Agent - Main agent implementation.

This module provides the main StarClawAgent class built on ReActAgent,
...
class StarClawAgent(ToolGuardMixin, ReActAgent):
    """StarClaw Agent with integrated tools, skills, and memory management.
    ...
    Python's MRO: StarClawAgent → ToolGuardMixin → ReActAgent.  If you
    ...
    **must** call ``super()._acting(...)`` / ``super()._reasoning(...)``
    ...
    def _rebuild_mcp_client(client: Any) -> Any | None:
        ...
        rebuild_info = getattr(client, "_starclaw_rebuild_info", None)
        ...
        try:
            if transport == "stdio":
                ...
                setattr(rebuilt_client, "_starclaw_rebuild_info", rebuild_info)
                return rebuilt_client
    ...
    def _strip_tool_use_from_msg(msg: Msg) -> Msg:
        ...
        if isinstance(msg.content, str):
            msg.content += StarClawAgent._ROUND_END_NOTICE
            return msg
        ...
        filtered.append({"type": "text", "text": StarClawAgent._ROUND_END_NOTICE})
        msg.content = filtered
        return msg
    ...
    async def reply(
        ...
        logger.info("StarClawAgent.reply: max_iters=%s", self.max_iters)
