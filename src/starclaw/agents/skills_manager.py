...
def _get_builtin_skill_names() -> list[str]:
    """Get list of builtin skill names from src/starclaw/agents/skills/."""
...
def read_skill_requirements(skill_dir: Path) -> SkillRequirements:
    ...
    elif "starclaw" in metadata:
        requires = metadata["starclaw"].get("requires", {})
    else:
...
def _skill_config_env_var_name(skill_name: str) -> str:
    ...
    return f"STARCLAW_SKILL_CONFIG_{''.join(normalized).strip('_') or 'DEFAULT'}"
...
def apply_skill_config_env_overrides(
    ...
    ``STARCLAW_SKILL_CONFIG_<SKILL_NAME>`` (JSON string).
    """
...
def _extract_zip_skills(data: bytes) -> tuple[Path, list[tuple[Path, str]]]:
    ...
    tmp_dir = Path(tempfile.mkdtemp(prefix="starclaw_skill_upload_"))
...
def _staged_skill_dir(skill_name: str) -> Iterator[Path]:
    ...
    temp_root = Path(
        tempfile.mkdtemp(prefix=f"starclaw_skill_stage_{skill_name}_"),
    )
