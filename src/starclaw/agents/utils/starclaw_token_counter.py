# -*- coding: utf-8 -*-
"""Token counting utilities for StarClaw using HuggingFace tokenizers.
...
class StarClawTokenCounter(HuggingFaceTokenCounter):
    """Token counter for StarClaw with configurable tokenizer support.
...
class StarClawEstimateTokenCounter(HuggingFaceTokenCounter):
...
_token_counter_cache: dict[
    tuple,
    StarClawTokenCounter | StarClawEstimateTokenCounter,
] = {}


def get_starclaw_token_counter(
    ...
    Args:
        ...
        use_estimate: If True (default), returns a StarClawEstimateTokenCounter
            that uses only character-based estimation without loading a
            tokenizer. If False, returns a full StarClawTokenCounter backed
    ...
    if use_estimate:
        return StarClawEstimateTokenCounter(
        ...
        if config_key not in _token_counter_cache:
            _token_counter_cache[config_key] = StarClawTokenCounter(
