...
from ..utils.logging import setup_logger, add_starclaw_file_handler
...
async def lifespan(
    ...
    add_starclaw_file_handler(WORKING_DIR / "starclaw.log")
...
# Console static dir: env, or starclaw package data (console), or cwd.
_CONSOLE_STATIC_ENV = "STARCLAW_CONSOLE_STATIC_DIR"


def _resolve_console_static_dir() -> str:
    if os.environ.get(_CONSOLE_STATIC_ENV):
        return os.environ[_CONSOLE_STATIC_ENV]
    # Shipped dist lives in starclaw package as static data
...
def read_root():
    ...
    return {
        "message": (
            "StarClaw Web Console is not available. "
            "If you installed StarClaw from source code, please run "
            "`npm ci && npm run build` in StarClaw's `console/` "
            "directory, and restart StarClaw to enable the "
            "web console."
        ),
    }
...
@app.get("/api/version")
def get_version():
    """Return the current StarClaw version."""
...
if os.path.isdir(_CONSOLE_STATIC_DIR):
    ...
    # @app.get("/starclaw-symbol.svg")
    # def _console_icon():
    #     f = _console_path / "starclaw-symbol.svg"
    #     if f.is_file():
    #         return FileResponse(f, media_type="image/svg+xml")
    #     raise HTTPException(status_code=404, detail="Not Found")

    # @app.get("/starclaw-dark.png")
    # def _console_dark_icon():
    #     f = _console_path / "starclaw-dark.png"
    #     if f.is_file():
    #         return FileResponse(f, media_type="image/png")
    #     raise HTTPException(status_code=404, detail="Not Found")
