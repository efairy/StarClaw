...
def migrate_legacy_workspace_to_default_agent() -> bool:
    ...
    default_agent_config = AgentProfileConfig(
        id="default",
        name="Default Agent",
        description="Default StarClaw agent (migrated from legacy config)",
...
def _migrate_workspace_items_from_source(
    ...
    Args:
        source_dir: Source directory (e.g., ~/.starclaw or WORKING_DIR)
...
def ensure_qa_agent_exists() -> None:
    ...
    ``starclaw_source_index``), and built-in tools are restricted (see
    ``build_qa_agent_tools_config``).
    ...
    agent_config = AgentProfileConfig(
        ...
        description=(
            "Builtin Q&A helper for StarClaw setup, local config under "
            "STARCLAW_WORKING_DIR, and documentation. Prefer reading files "
