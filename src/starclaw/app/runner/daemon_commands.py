...
Shared by in-chat /daemon <sub> and CLI `starclaw daemon <sub>`.
Logs: tail WORKING_DIR / "starclaw.log". Restart: in-process reload of channels,
...
async def run_daemon_restart(context: DaemonContext) -> str:
    ...
    return (
        ...
        "Run the app (e.g. `starclaw app`) and use /daemon restart in chat, "
...
def run_daemon_version(context: DaemonContext) -> str:
    ...
    return (
        ...
        f"- Log file: {context.working_dir / 'starclaw.log'}"
    )


def run_daemon_logs(context: DaemonContext, lines: int = 100) -> str:
    """Tail last N lines from WORKING_DIR / starclaw.log."""
    log_path = context.working_dir / "starclaw.log"
