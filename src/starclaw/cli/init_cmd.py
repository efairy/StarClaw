...
StarClaw is a personal assistant that runs in your own environment. It can connect to
...
If multiple people can message the same StarClaw instance with tools enabled, they
share the same delegated authority (files, commands, secrets the agent can use).

If you are not comfortable with access control and hardening, do not run StarClaw with
...
TELEMETRY_INFO = """
Help improve StarClaw by sharing anonymous usage data!

We collect only:
• StarClaw version (e.g., 0.0.7)
...
This helps us understand StarClaw's usage environment and prioritize improvements.
"""
...
def _echo_telemetry_info_box() -> None:
    ...
    console.print(
        Panel(
            ...
            title="[bold]📊 Help improve StarClaw[/bold]",
            border_style="blue",
        ),
    )
