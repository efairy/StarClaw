...
def _matches_starclaw_cli_command(command: str, *subcommands: str) -> bool:
    """Return whether command line looks like a StarClaw CLI invocation."""
    lowered = f" {command.lower()}"
    return any(
        pattern in lowered
        for subcommand in subcommands
        for pattern in (
            f" -m starclaw {subcommand}",
            f" starclaw {subcommand}",
            f"__main__.py {subcommand}",
            f'starclaw.exe" {subcommand}',
            f"starclaw.exe {subcommand}",
        )
    )


def _is_starclaw_service_command(command: str) -> bool:
    """Return whether the command line looks like a local StarClaw app."""
    return _matches_starclaw_cli_command(command, "app")


def _is_starclaw_wrapper_process(name: str, command: str) -> bool:
    """Return whether the process looks like a StarClaw CLI wrapper."""
    lowered_name = name.lower().removesuffix(".exe")
    return lowered_name == "starclaw" or _matches_starclaw_cli_command(
        command,
        "app",
        "desktop",
    )
...
def _candidate_hosts(host: str | None) -> list[str]:
    """Return host variants that can reach a local StarClaw service."""
    if not host:
        return []
