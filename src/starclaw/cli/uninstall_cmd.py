# -*- coding: utf-8 -*-
"""starclaw uninstall — remove the StarClaw environment and CLI wrapper."""
...
def _remove_path_entry(profile: Path) -> bool:
    """
    Remove StarClaw PATH lines from a shell profile. Returns True if changed.
    """
    if not profile.is_file():
        return False

    text = profile.read_text()
    # Remove the "# StarClaw" comment line and the export PATH line
    cleaned = re.sub(
        r"\n?# StarClaw\nexport PATH=\"\$HOME/\.starclaw/bin:\$PATH\"\n?",
        "\n",
        text,
    )
    if cleaned == text:
...
def uninstall_cmd(purge: bool, yes: bool) -> None:
    """Remove StarClaw environment, CLI wrapper, and shell PATH entries."""
    wd = WORKING_DIR

    if purge:
        click.echo(f"This will remove ALL StarClaw data in {wd}")
    else:
        click.echo(
            "This will remove the StarClaw Python environment and CLI wrapper.",
        )
    ...
    click.echo("")
    click.echo("StarClaw uninstalled. Please restart your terminal.")
