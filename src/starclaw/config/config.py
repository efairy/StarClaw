...
class VoiceChannelConfig(BaseChannelConfig):
    ...
    language: str = "en-US"
    welcome_greeting: str = "Hi! This is StarClaw. How can I help you?"
...
class WeixinConfig(BaseChannelConfig):
    ...
    bot_token_file: Path to persist/load the bot_token
                    (default ~/.starclaw/weixin_bot_token).
...
def load_agent_config(agent_id: str) -> AgentProfileConfig:
    ...
    # This keeps STARCLAW_WORKING_DIR effective even if existing agent.json
    # contains older hard-coded paths like "~/.starclaw/media".
    try:
...
def migrate_legacy_config_to_multi_agent() -> bool:
    ...
    default_agent_config = AgentProfileConfig(
        id="default",
        name="Default Agent",
        description="Default StarClaw agent",
    ...
    # When STARCLAW_WORKING_DIR is customized, historical data may still exist
    # under "~/.starclaw".
    old_workspace = Path("~/.starclaw").expanduser().resolve()
