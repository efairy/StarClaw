...
WORKING_DIR = (
    Path(EnvVarLoader.get_str("STARCLAW_WORKING_DIR", "~/.starclaw"))
    .expanduser()
    .resolve()
)
SECRET_DIR = (
...
JOBS_FILE = EnvVarLoader.get_str("STARCLAW_JOBS_FILE", "jobs.json")

CHATS_FILE = EnvVarLoader.get_str("STARCLAW_CHATS_FILE", "chats.json")

# Builtin multi-agent profile: StarClaw Q&A helper.
BUILTIN_QA_AGENT_ID = "StarClaw_QA_Agent_0.1beta1"
...
BUILTIN_QA_AGENT_SKILL_NAMES: tuple[str, ...] = (
    "guidance",
    "starclaw_source_index",
)

TOKEN_USAGE_FILE = EnvVarLoader.get_str(
    "STARCLAW_TOKEN_USAGE_FILE",
    "token_usage.json",
)

CONFIG_FILE = EnvVarLoader.get_str("STARCLAW_CONFIG_FILE", "config.json")

HEARTBEAT_FILE = EnvVarLoader.get_str("STARCLAW_HEARTBEAT_FILE", "HEARTBEAT.md")
...
DOCS_ENABLED = EnvVarLoader.get_bool("STARCLAW_OPENAPI_DOCS", False)

# Memory directory
MEMORY_DIR = WORKING_DIR / "memory"

# Custom channel modules (installed via `starclaw channels install`); manager
...
MEMORY_COMPACT_KEEP_RECENT = EnvVarLoader.get_int(
    "STARCLAW_MEMORY_COMPACT_KEEP_RECENT",
    3,
    min_value=0,
)
...
# Example: STARCLAW_CORS_ORIGINS="http://localhost:5173,http://127.0.0.1:5173"
# When unset, CORS middleware is not applied.
CORS_ORIGINS = EnvVarLoader.get_str("STARCLAW_CORS_ORIGINS", "").strip()

# LLM API retry configuration
LLM_MAX_RETRIES = EnvVarLoader.get_int(
    "STARCLAW_LLM_MAX_RETRIES",
    3,
    min_value=0,
)

LLM_BACKOFF_BASE = EnvVarLoader.get_float(
    "STARCLAW_LLM_BACKOFF_BASE",
    1.0,
    min_value=0.1,
)

LLM_BACKOFF_CAP = EnvVarLoader.get_float(
    "STARCLAW_LLM_BACKOFF_CAP",
    10.0,
    min_value=0.5,
)
...
LLM_RATE_LIMIT_JITTER = EnvVarLoader.get_float(
    "STARCLAW_LLM_RATE_LIMIT_JITTER",
    1.0,
    min_value=0.0,
)
...
try:
    TOOL_GUARD_APPROVAL_TIMEOUT_SECONDS = max(
        float(
            os.environ.get("STARCLAW_TOOL_GUARD_APPROVAL_TIMEOUT_SECONDS", "600"),
        ),
        1.0,
    )
except (TypeError, ValueError):
