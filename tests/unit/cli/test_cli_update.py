...
from starclaw.__version__ import __version__
from starclaw.cli.main import cli
from starclaw.cli.update_cmd import (
    InstallInfo,
    RunningServiceInfo,
...
def _install_info(
    *,
    ...
    return InstallInfo(
        package_dir="/tmp/site-packages/starclaw",
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            {
                "url": "file:///Users/test/StarClaw",
                "dir_info": {"editable": True},
            },
            ("editable", "file:///Users/test/StarClaw"),
        ),
        (
            {
                "url": "https://github.com/efairy/StarClaw.git",
                "vcs_info": {"vcs": "git", "commit_id": "abc123"},
            },
            ("vcs", "https://github.com/efairy/StarClaw.git"),
        ),
        (
            {"url": "file:///tmp/starclaw.whl"},
            ("local", "file:///tmp/starclaw.whl"),
        ),
        (
            {"url": "https://example.com/starclaw.whl"},
            ("direct-url", "https://example.com/starclaw.whl"),
        ),
    ],
)
...
def test_detect_installation(
    ...
    from starclaw.cli import update_cmd as update_cmd_module
...
def test_update_reports_up_to_date(monkeypatch) -> None:
    ...
    assert "StarClaw is already up to date." in result.output
...
def test_probe_service_ignores_proxy_env(monkeypatch) -> None:
    ...
    monkeypatch.setattr("starclaw.cli.update_cmd.httpx.get", _fake_get)
...
def test_update_blocks_running_service(monkeypatch) -> None:
    ...
    assert result.exit_code != 0
    assert "Please stop it before running `starclaw update`" in result.output
    assert (
        "without `--yes` to confirm a forced `starclaw shutdown`" in result.output
    )
...
def test_update_can_cancel_forced_shutdown(monkeypatch) -> None:
    ...
    assert result.exit_code == 0
    assert (
        "forcibly terminate the current StarClaw backend/frontend "
...
def test_update_can_force_shutdown_running_service(
    ...
    def _fake_shutdown(
        ...
        class _Result:
            returncode = 0
            stdout = "Stopped StarClaw processes: 1234\n"
    ...
    assert result.exit_code == 0
    assert "Running `starclaw shutdown` before updating..." in result.output
    assert "Stopped StarClaw processes: 1234" in result.output
    assert "Starting StarClaw update..." in result.output
...
def test_update_worker_waits_for_launcher_exit(
    ...
    monkeypatch.setattr(
        "starclaw.cli.update_cmd._wait_for_process_exit",
...
def test_run_update_worker_detached_spawns_without_capture(
    ...
    monkeypatch.setattr(
        "starclaw.cli.update_cmd._spawn_update_worker",
        _fake_spawn,
    )
...
def test_update_worker_foreground_streams_output_and_cleans_plan(
    ...
    assert return_code == 0
    assert "[starclaw] Updating StarClaw 1.0.0 -> 9.9.9..." in captured.out
    assert "[starclaw] Using installer: integration-test" in captured.out
    assert "installer: preparing" in captured.out
    assert "installer: done" in captured.out
    assert "[starclaw] Update completed successfully." in captured.out
...
def test_update_worker_foreground_propagates_failure_exit_code(
    ...
    assert "[starclaw] Update failed with exit code 7." in captured.out
    assert not plan_path.exists()
