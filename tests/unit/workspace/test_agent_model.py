...
from starclaw.config.config import (
    AgentProfileConfig,
    AgentsRunningConfig,
    load_agent_config,
    save_agent_config,
)
from starclaw.constant import (
    LLM_BACKOFF_BASE,
    LLM_BACKOFF_CAP,
    LLM_MAX_RETRIES,
)
...
def mock_agent_workspace(tmp_path, monkeypatch):
    ...
    import json
    from starclaw.config.utils import get_config_path
    from starclaw.config.config import Config, AgentsConfig, AgentProfileRef

    # Setup workspace directory
    workspace_dir = tmp_path / "workspaces" / "test_agent"
    workspace_dir.mkdir(parents=True, exist_ok=True)

    # Patch WORKING_DIR so tests never touch the real ~/.starclaw/
    monkeypatch.setattr("starclaw.config.utils.WORKING_DIR", tmp_path)
    monkeypatch.setattr("starclaw.config.config.WORKING_DIR", tmp_path)
