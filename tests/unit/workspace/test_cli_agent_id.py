...
from starclaw.cli.channels_cmd import channels_group
from starclaw.cli.cron_cmd import cron_group
from starclaw.cli.daemon_cmd import daemon_group
from starclaw.cli.chats_cmd import chats_group
from starclaw.cli.skills_cmd import skills_group
from starclaw.config.config import AgentProfileConfig
...
def test_channels_list_default_agent(
    ...
    """Test starclaw channels list uses default agent by default."""
    ...
    with patch("starclaw.cli.channels_cmd.load_agent_config") as mock_load:
...
def test_channels_list_custom_agent(
    ...
    """Test starclaw channels list with custom agent_id."""
...
def test_cron_list_with_agent_id():
    """Test starclaw cron list with --agent-id."""
    runner = CliRunner()

    with patch("starclaw.cli.cron_cmd.client") as mock_client:
...
def test_daemon_status_default_agent():
    """Test starclaw daemon status defaults to 'default' agent."""
    runner = CliRunner()

    with patch("starclaw.cli.daemon_cmd.run_daemon_status") as mock_status:
        with patch("starclaw.cli.daemon_cmd._get_agent_workspace") as mock_ws:
...
def test_daemon_status_custom_agent():
    """Test starclaw daemon status with custom agent."""
    runner = CliRunner()
...
def test_skills_list_default_agent():
    """Test starclaw skills list defaults to 'default' agent."""
    runner = CliRunner()

    with patch(
        "starclaw.cli.skills_cmd._get_agent_workspace",
    ) as mock_ws:
        with patch("starclaw.cli.skills_cmd.SkillService") as mock_service:
            mock_ws.return_value = "/tmp/default"
...
def test_skills_list_custom_agent():
    """Test starclaw skills list with custom agent."""
    runner = CliRunner()
...
def test_chats_list_with_agent_id():
    """Test starclaw chats list with --agent-id."""
    runner = CliRunner()

    with patch("starclaw.cli.chats_cmd.client") as mock_client:
        mock_response = MagicMock()
