...
A **channel** is where you talk to StarClaw: connect DingTalk and it replies
...
- **Edit `agent.json` directly** — Agent workspace config at `~/.starclaw/workspaces/{agent_id}/agent.json`, set `enabled: true` and fill in that platform's credentials. Saving triggers a reload without restarting the app.
...
In your agent's `agent.json` (e.g., `~/.starclaw/workspaces/default/agent.json`), find `channels.dingtalk` and fill in the corresponding information, for example:
...
> - `robot_code` is recommended in group scenarios; if empty, StarClaw falls back to `client_id`.

Save the file; if the app is already running, the channel will reload. Otherwise run `starclaw app`.
...
4. Run **`starclaw app`** to start StarClaw
...
> **Note:** Follow this order: Configure App ID/Secret → start `starclaw app` → then configure the long connection in the Feishu console. If errors persist, try stopping the starclaw service and restarting `starclaw app`.
...
Find `channels.feishu` in your agent's `agent.json` (e.g., `~/.starclaw/workspaces/default/agent.json`). Only **App ID** and **App Secret** are required (copy from the Feishu console under Credentials & basic info):
...
> **Note:** You can also fill in **App ID** and **App Secret** in the Console UI, but you must restart the starclaw service before continuing with the long-connection configuration.
...
3. For StarClaw to read iMessage data, **Terminal** (or the app you use to run `starclaw app`) and **Messages** need **Full Disk Access** (System Settings → Privacy & Security → Full Disk Access).
...
4. Set the iMessage database path. The default is `~/Library/Messages/chat.db`; use this unless you've moved the database. You can configure it in either of these ways:
   ...
   - Or edit the agent workspace `agent.json` (usually at `~/.starclaw/workspaces/default/agent.json`):
...
Find `channels.discord` in your agent's `agent.json` (e.g., `~/.starclaw/workspaces/default/agent.json`) and fill in the fields, for example:
...
5. In **Developer settings**, get **AppID** and **AppSecret** (ClientSecret) and fill them into config (see below). Add your server’s **IP to the whitelist** — only whitelisted IPs can call the Open API outside sandbox.
   ...
   > **Tip:** If you are using ModelScope Creative Space to deploy StarClaw, the IP whitelist for QQ channel should be: `47.92.200.108`
...
In your agent's `agent.json` (e.g., `~/.starclaw/workspaces/default/agent.json`), find `channels.qq` and set `app_id` and `client_secret` to the values above:
...
**Method 2:** Fill in `agent.json` (e.g., `~/.starclaw/workspaces/default/agent.json`)
...
| `media_dir`              | string | `~/.starclaw/media` | Media files (images, files, etc.) download directory |
| `max_reconnect_attempts` | int    | `-1`             | WebSocket max reconnect attempts (`-1` = unlimited)  |
...
- **Authentication**: On first use, scan a QR code to authorize. The token is automatically persisted to a local file (default `~/.starclaw/weixin_bot_token`), so you won't need to scan again on subsequent starts.
...
1. Open the StarClaw Web Console and go to **Settings → Channels → WeChat Personal (iLink)**.
...
You can also configure directly in the agent workspace `agent.json` (e.g., `~/.starclaw/workspaces/default/agent.json`):

```json
"weixin": {
  "enabled": true,
  "bot_token": "your_bot_token",
  "bot_token_file": "~/.starclaw/weixin_bot_token",
  "base_url": "",
  "media_dir": "~/.starclaw/media",
  "dm_policy": "open",
  "group_policy": "open"
...
| `bot_token_file` | string | `~/.starclaw/weixin_bot_token` | Path to persist the token for future runs                                             |
| `base_url`       | string | official default            | iLink API base URL; leave empty to use the official default                           |
| `media_dir`      | string | `~/.starclaw/media`            | Directory to save received images and files                                           |
...
WEIXIN_BOT_TOKEN_FILE=~/.starclaw/weixin_bot_token
WEIXIN_MEDIA_DIR=~/.starclaw/media
...
Find `channels.telegram` in your agent's `agent.json` (e.g., `~/.starclaw/workspaces/default/agent.json`) and fill in the fields, for example:
...
2. Fuzzy match subscription and automatic push
   ...
   Subscribe to the wildcard topic `/server/+/up`. Messages will be automatically pushed to the corresponding topic based on the client's `client_id`. For example, after a client pushes a message to `/server/client_a/up`, StarClaw will push the message to `/client/client_b/down` after processing.
...
3. Redirected topic push
   ...
   Messages will be pushed to `client/client_b/down` based on the `redirect_client_id` attribute, enabling cross-topic push. In IoT scenarios, with StarClaw as the core, autonomous message pushing between multiple devices can be achieved according to individual requirements.

---

## Matrix

The Matrix channel connects StarClaw to any Matrix homeserver using the [matrix-nio](https://github.com/poljar/matrix-nio) library. It supports text messaging in both direct messages and group rooms.
...
Find `channels.matrix` in your agent's `agent.json` (e.g., `~/.starclaw/workspaces/default/agent.json`):
...
Save the file; the channel will reload automatically if StarClaw is already running.
...
The XiaoYi channel connects StarClaw via **A2A (Agent-to-Agent) protocol** over WebSocket to Huawei's AI assistant platform.
...
The Voice channel enables phone call interactions with StarClaw via Twilio ConversationRelay, supporting Speech-to-Text (STT) and Text-to-Speech (TTS) for voice-based conversations.

### Prerequisites

1. **Twilio Account**: Register at [Twilio](https://www.twilio.com/) and obtain credentials
2. **Cloudflare Tunnel** (or similar): Expose your local StarClaw service to the public internet for Twilio webhook callbacks
...
Twilio needs to reach StarClaw's webhook endpoint via the public internet, so you need to expose your local service.
...
```json
{
  "channels": {
    "voice": {
      ...
      "language": "en-US",
      "welcome_greeting": "Hi! This is StarClaw. How can I help you?"
    }
  }
}
```
...
After configuration, simply call your Twilio phone number to have a voice conversation with StarClaw:

1. Dial the phone number
2. After hearing the welcome greeting, start speaking
3. StarClaw converts speech to text and processes it through the Agent
...
| `welcome_greeting`   | string | `"Hi! This is StarClaw. How can I help you?"` | Welcome message when call connects           |
...
from starclaw.app.channels.base import BaseChannel
from starclaw.app.channels.schema import ChannelType
...
- **Directory**: Channels under the working dir at `custom_channels/` (default `~/.starclaw/custom_channels/`) are loaded at runtime. The manager scans `.py` files and packages (subdirs with `__init__.py`), loads `BaseChannel` subclasses, and registers them by the class’s `channel` attribute.
- **Install**: `starclaw channels install <key>` creates a template `<key>.py` in `custom_channels/` for you to edit, or use `--path <local path>` / `--url <URL>` to copy a channel module from disk or the web. `starclaw channels add <key>` does the same and also adds a default entry to config (with optional `--path`/`--url`).
- **Remove**: `starclaw channels remove <key>` deletes that channel’s module from `custom_channels/` (custom channels only; built-ins cannot be removed). By default it also removes the key from `channels` in `config.json`; use `--keep-config` to leave config unchanged.
- **Config**: `ChannelConfig` uses `extra="allow"`, so any channel key can appear under `channels` in `config.json`. Use `starclaw channels config` for interactive setup or edit config by hand.

### HTTP route registration

For channels that require webhook callbacks (e.g., WeChat, Slack, LINE), you can register custom HTTP routes by exporting a `register_app_routes` callable in your module — no changes to StarClaw's core source required.

At startup, StarClaw scans modules in `custom_channels/` for a `register_app_routes` export. If found, it is called with the FastAPI `app` instance, allowing the channel to register any routes it needs.
...
def register_app_routes(app):
    ...
    async def echo_callback(request):
        ...
        from starclaw.app.channels.base import TextContent
...
**Real-world example**: WeChat ClawBot integration ([PR #2140](https://github.com/efairy/StarClaw/pull/2140), [Issue #2043](https://github.com/efairy/StarClaw/issues/2043)) uses this mechanism to register the `/api/wechat/callback` route with Tencent's official SDK for message delivery.
