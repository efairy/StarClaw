...
From **v0.1.0**, StarClaw supports **multi-agent**. Configuration is split into two layers:
...
The default working directory is `~/.starclaw`. After running `starclaw init`, the complete structure looks like:

```
$STARCLAW_WORKING_DIR/                      # Default ~/.starclaw
...
$STARCLAW_SECRET_DIR/                       # Default ~/.starclaw.secret
├── providers.json                       # Model provider config and API keys
└── envs.json                            # Environment variables
```

> **Path explanation:** `$STARCLAW_WORKING_DIR` and `$STARCLAW_SECRET_DIR` are environment variables, with default values of `~/.starclaw` and `~/.starclaw.secret` respectively. They can be customized via environment variables, see "Environment Variables" section below.

### Directory Explanation

**Global Directory (`~/.starclaw/`)**
...
**Agent Workspace (`~/.starclaw/workspaces/{agent_id}/`)**
...
> **Persona files:** Agent behavior and personality are defined by persona files. Running `starclaw init` automatically creates template files based on your chosen language (`zh` / `en` / `ru`). For detailed explanation and management, see [Agent Persona](./persona).
...
| ------------------------ | ------------------ | ----------------------------------------------------------------------------------------------------------- |
| `STARCLAW_WORKING_DIR`      | `~/.starclaw`         | Working directory root path                                                                                 |
| `STARCLAW_SECRET_DIR`       | `~/.starclaw.secret`  | Sensitive data directory (stores `providers.json` and `envs.json`). Docker default is `/app/working.secret` |
| `STARCLAW_CONFIG_FILE`      | `config.json`      | Config file name (relative to `STARCLAW_WORKING_DIR`)                                                          |
| `STARCLAW_HEARTBEAT_FILE`   | `HEARTBEAT.md`     | Heartbeat file name (relative to agent workspace)                                                           |
| `STARCLAW_JOBS_FILE`        | `jobs.json`        | Cron jobs file name (relative to agent workspace)                                                           |
| `STARCLAW_CHATS_FILE`       | `chats.json`       | Conversation history file name (relative to agent workspace)                                                |
| `STARCLAW_TOKEN_USAGE_FILE` | `token_usage.json` | Token usage record file name (relative to agent workspace)                                                  |

**Other configuration:**

| Variable                           | Default         | Description                                                                 |
| ---------------------------------- | --------------- | --------------------------------------------------------------------------- |
| `STARCLAW_LOG_LEVEL`                  | `info`          | Log level (`debug` / `info` / `warning` / `error` / `critical`)             |
| `STARCLAW_MEMORY_COMPACT_THRESHOLD`   | `100000`        | Character threshold to trigger memory compaction                            |
| `STARCLAW_MEMORY_COMPACT_KEEP_RECENT` | `3`             | Number of recent messages to keep after compaction                          |
| `STARCLAW_MEMORY_COMPACT_RATIO`       | `0.7`           | Threshold ratio for triggering compaction (relative to context window size) |
| `STARCLAW_CONSOLE_STATIC_DIR`         | _(auto-detect)_ | Console frontend static files path                                          |

**Security & Authentication:**

| Variable                   | Default | Description                                        |
| -------------------------- | ------- | -------------------------------------------------- |
| `STARCLAW_AUTH_ENABLED`       | `false` | Whether to enable Web console login authentication |
| `STARCLAW_AUTH_USERNAME`      | -       | Admin username for auto-registration (optional)    |
| `STARCLAW_AUTH_PASSWORD`      | -       | Admin password for auto-registration (optional)    |
| `STARCLAW_TOOL_GUARD_ENABLED` | `true`  | Whether to enable tool guard                       |
| `STARCLAW_SKILL_SCAN_MODE`    | `warn`  | Skill scanning mode (`block` / `warn` / `off`)     |
...
```bash
export STARCLAW_WORKING_DIR=/home/me/my_starclaw
starclaw app
```
...
1. **Global config** - `~/.starclaw/config.json` (providers, environment variables, agent list)
2. **Agent config** - `~/.starclaw/workspaces/{agent_id}/agent.json` (per-agent settings)
...
| `last_api.host`       | string \| null | `null`              | Host address from last `starclaw app` start                          |
| `last_api.port`       | int \| null    | `null`              | Port from last `starclaw app` start                                  |
...
| `workspace_dir` | string | No       | Workspace path (optional, defaults to `$STARCLAW_WORKING_DIR/workspaces/{id}`) |
...
> **Model provider configuration** is stored in `$STARCLAW_SECRET_DIR/providers.json` (default `~/.starclaw.secret/providers.json`).
> **Environment variables** are stored in `$STARCLAW_SECRET_DIR/envs.json` (default `~/.starclaw.secret/envs.json`).

### Agent config (agent.json)

Each agent has an independent `agent.json` in its workspace directory (`~/.starclaw/workspaces/{agent_id}/`) that stores all of its configuration (channels, tools, heartbeat, MCP, security, etc.). This allows different agents to have completely different configurations without interfering with each other.
...
StarClaw needs an LLM provider to work. You can set it up in three ways:

- **`starclaw init`** — interactive wizard, the easiest way
...
> **Tip:** Run `starclaw init` and follow the prompts — it will list available
...
> StarClaw does not verify whether the key is correct or has sufficient quota —
...
- **`starclaw init`** — prompts "Configure environment variables?" during setup
...
> keys) are valid. StarClaw only stores and injects them — it does not verify
> correctness.
...
| Directory                                | Purpose                                           |
| ---------------------------------------- | ------------------------------------------------- |
| `~/.starclaw/skill_pool/`                   | Local shared pool for built-ins and shared skills |
| `~/.starclaw/workspaces/{agent_id}/skills/` | Skills present in a specific agent's workspace    |
...
Skill enabled state and configuration are controlled by `~/.starclaw/workspaces/{agent_id}/skill.json`.

**Manage skills via:**

- Console (Agent → Skills) — Visual management, import, create, enable/disable
- `starclaw init` (choose all / none / custom during setup)
- `starclaw skills config` (interactive toggle)
...
StarClaw has persistent cross-conversation memory: it automatically compresses context and saves key information to Markdown files for long-term retention.

Memory files are stored in the agent workspace:

| File / Directory                                      | Purpose                                                               |
| ----------------------------------------------------- | --------------------------------------------------------------------- |
| `~/.starclaw/workspaces/{agent_id}/MEMORY.md`            | Long-lived key information (decisions, preferences, persistent facts) |
| `~/.starclaw/workspaces/{agent_id}/memory/YYYY-MM-DD.md` | Daily logs (notes, runtime context, auto-generated summaries)         |
...
- Everything lives under **`~/.starclaw`** by default; override with `STARCLAW_WORKING_DIR` (and related env vars) if needed.
- From **v0.1.0**, configuration is split into:
  - **Global config** (`~/.starclaw/config.json`) — providers, environment variables, agent list
  - **Agent config** (`~/.starclaw/workspaces/{agent_id}/agent.json`) — per-agent settings
...
- LLM providers are globally configured via `starclaw init` or the Console.
