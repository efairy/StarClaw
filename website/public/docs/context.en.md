...
StarClaw context management uses two parallel offload paths to handle the limited context window:
...
StarClaw's context consists of two components:
...
When the context approaches its limit, StarClaw automatically triggers compaction, condensing old conversations into a structured summary.
...
Configuration is located in `~/.starclaw/config.json` under `agents.running`:
