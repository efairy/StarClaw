# StarClaw Desktop Application Guide
...
This guide explains how to install and use the StarClaw Desktop application on Windows and macOS.

[releases]: https://github.com/efairy/StarClaw/releases
...
1. **Download the installer**
   Download `StarClaw-Setup-<version>.exe` from the [Release page][releases]

2. **Run the installer**
   Double-click the `.exe` file and follow the installation wizard
   - Default installation location: `C:\Users\<your-username>\AppData\Local\StarClaw`
...
#### **StarClaw Desktop** (Recommended for daily use)
...
- **How to Launch**: Double-click the "StarClaw Desktop" icon on desktop or Start Menu
- **Technical Note**: Uses VBScript launcher, runs Python process in background

#### **StarClaw Desktop (Debug)** (Debug Mode)
...
- **How to Launch**: Double-click "StarClaw Desktop (Debug)" icon in Start Menu
- **Log Contents**:
...
A: Use "StarClaw Desktop (Debug)" mode to view terminal output for error messages

**Q: How to uninstall?**
A: Go to Windows Settings → Apps → Installed apps → Find "StarClaw Desktop" → Uninstall
...
1. **Download the archive**
   Download `StarClaw-<version>-macOS.zip` from the [Release page][releases]

2. **Extract**
   Double-click the `.zip` file to extract and get `StarClaw.app`

3. **Move to Applications folder (Optional)**
   Drag `StarClaw.app` to the `/Applications` folder

### First Launch: Bypassing System Security Restrictions

#### Why manual trust is needed?

StarClaw is **not Apple Developer-signed or notarized**, so macOS Gatekeeper will block it by default.
...
1. **Right-click** (or Control + click) on `StarClaw.app`
...
2. Scroll down to find a message like:
   _"'StarClaw' was blocked from use because it cannot verify the developer"_
...
xattr -cr /Applications/StarClaw.app
```
...
- Double-click `StarClaw.app` to launch
- The app runs in the background and opens a browser window
- Logs are written to: `~/.starclaw/desktop.log`
...
cd /Applications  # or wherever your StarClaw.app is located

# Set environment variables and launch
APP_ENV="$(pwd)/StarClaw.app/Contents/Resources/env"
PYTHONPATH= PYTHONHOME="$APP_ENV" "$APP_ENV/bin/python" -m starclaw desktop
```
...
1. Check the `~/.starclaw/desktop.log` file for errors
...
**Q: How to uninstall?**
A: Drag `StarClaw.app` to the Trash, then delete the `~/.starclaw` configuration folder
...
- **GitHub Issues**: [Submit an issue](https://github.com/efairy/StarClaw/issues)
- **Packaging documentation**: `scripts/pack/README.md` - Technical details and local build guide
- **Log locations**:
  - Windows: View in Debug mode terminal, or `%USERPROFILE%\.starclaw\` directory
  - macOS: `~/.starclaw/desktop.log`
