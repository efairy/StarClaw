...
StarClaw 支持**多智能体**，允许您在同一个 StarClaw 实例中运行多个独立的 AI 智能体。
...
简单来说，**多智能体**就是让您可以在一个 StarClaw 中运行多个"分身"，每个分身：
...
启动 StarClaw 后，在控制台**右上角**可以看到**智能体切换器**：
...
**注意**：删除后工作区目录会保留（防止误删数据），如需彻底清理，请手动删除 `~/.starclaw/workspaces/{agent_id}` 目录。
...
如果您之前使用 StarClaw **v0.0.x**，升级到 **v0.1.0** 时会**自动迁移**：
...
- `starclaw channels` - 频道管理
- `starclaw cron` - 定时任务
- `starclaw daemon` - 运行状态
- `starclaw chats` - 对话管理
- `starclaw skills` - 技能管理

**不支持 `--agent-id` 的命令**（全局操作）：

- `starclaw init` - 初始化
- `starclaw providers` - 模型提供商
- `starclaw models` - 模型配置
- `starclaw env` - 环境变量
