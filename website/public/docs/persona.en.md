...
StarClaw defines an agent's "persona" through a set of Markdown files that are loaded into the system prompt. These files determine the agent's behavioral style, working approach, and personality traits. By editing these files, you can shape the agent into your ideal assistant—whether that's a meticulous work aide, a warm life companion, or a technical expert.

---

## Persona Files

Agent persona files are Markdown documents stored in the agent's workspace directory. The workspace location is determined by the `STARCLAW_WORKING_DIR` environment variable (defaults to `~/.starclaw`), with the full path being:

```
$STARCLAW_WORKING_DIR/workspaces/{agent_id}/
```
...
When running `starclaw init` for the first time, BOOTSTRAP.md is automatically created to guide the initial "conversation" between user and agent, establishing identity, preferences, and style. Once complete, the agent writes the configuration to PROFILE.md and SOUL.md, then deletes BOOTSTRAP.md.
...
You can also directly edit the `system_prompt_files` field in the agent configuration file (`~/.starclaw/workspaces/{agent_id}/agent.json`) to manage persona file loading:
...
Running `starclaw init` automatically creates template files based on your chosen language (`zh` / `en` / `ru`):
...
If using `starclaw init --defaults`, the default language is `zh` (Chinese).
...
StarClaw automatically creates a built-in agent named **"QA Agent"** (ID: `StarClaw_QA_Agent_0.1beta1`) when you first run `starclaw init`.

### QA Agent Features

This is an agent **specifically designed to answer StarClaw-related questions**:

- **Dedicated persona**: Uses persona files optimized for Q&A (different from regular agents)
- **Pre-installed skills**: Auto-enables `guidance` and `starclaw_source_index` skills for querying StarClaw documentation and source code
...
- **Auto-maintained**: Running `starclaw init` ensures this agent exists

### How to Use?

You can select "QA Agent" in the agent switcher at the top-right of the Console, then ask it any questions about StarClaw.
...
- **Can delete**: Delete it in "Settings → Agent Management" (doesn't affect other agents; will be recreated on next `starclaw init`)
- **Workspace location**: `$STARCLAW_WORKING_DIR/workspaces/StarClaw_QA_Agent_0.1beta1/` (defaults to `~/.starclaw/workspaces/StarClaw_QA_Agent_0.1beta1/`)
