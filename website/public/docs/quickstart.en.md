...
This section describes six ways to run StarClaw:
...
> 💡 **After install & start**: Before configuring channels, you can open the [Console](./console) (`http://127.0.0.1:8088/`) to chat with StarClaw and configure the agent. When you're ready to chat in DingTalk, Feishu, QQ, etc., head to [Channels](./channels) to add a channel.
...
Then open a new terminal (the installer adds StarClaw to your PATH automatically).
...
To upgrade, simply re-run the install command. To uninstall, run `starclaw uninstall`.

### Step 2: Init

Generate `config.json` and `HEARTBEAT.md` in the working directory (default
`~/.starclaw`). Two options:
...
To overwrite existing config, use `starclaw init --force` (you will be prompted).
...
configured a channel, StarClaw will reply there; otherwise you can add one after
...
`source .venv/bin/activate` on Linux/macOS or `.venv\Scripts\Activate.ps1` on Windows). This installs the `starclaw` command.
...
If you're not comfortable with command-line tools, you can download and use StarClaw's desktop application without manually configuring Python environments or running commands.
...
1. **Download the installer**
   Go to [GitHub Releases](https://github.com/efairy/StarClaw/releases) to download the version for your system:

   - Windows: `StarClaw-Setup-<version>.exe`
   - macOS: `StarClaw-<version>-macOS.zip`

2. **Install and Launch**

   - **Windows**: Double-click the `.exe` file to install following the wizard, then double-click the desktop shortcut to launch
   - **macOS**: Extract the `.zip` to get `StarClaw.app`, first time requires right-click and select "Open" to bypass system security restrictions
...
If you prefer not to install Python locally, you can deploy StarClaw to ModelScope Studio's cloud:

1. First, sign up and log in at [ModelScope](https://modelscope.cn/register?back=%2Fhome);
2. Open the [StarClaw Studio](https://modelscope.cn/studios/fork?target=AgentScope/StarClaw) and complete the one-click setup.
...
Images are on **Docker Hub** (`agentscope/starclaw`). Image tags: `latest` (stable); `pre` (PyPI pre-release). Also available on Alibaba Cloud ACR for users in China: `agentscope-registry.ap-southeast-1.cr.aliyuncs.com/agentscope/starclaw` (same tags).
...
Then open **http://127.0.0.1:8088/** in your browser for the Console. Config, memory, and skills are stored in the `starclaw-data` volume; model provider settings and API keys are in the `starclaw-secrets` volume. To pass API keys, add `-e DASHSCOPE_API_KEY=xxx` or `--env-file .env` to `docker run`.

---

## Option F: Deploy on Alibaba Cloud ECS

To run StarClaw on Alibaba Cloud, use the ECS one-click deployment:

1. Open the [StarClaw on Alibaba Cloud (ECS) deployment link](https://computenest.console.aliyun.com/service/instance/create/cn-hangzhou?type=user&ServiceId=service-1ed84201799f40879884) and fill in the parameters as prompted;
...
- **Chat with StarClaw** — [Channels](./channels): connect one channel
